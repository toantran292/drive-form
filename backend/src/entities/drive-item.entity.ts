...
export class DriveItem {
    ...
    async updateDescendantsInfo(queryRunner?: QueryRunner) {
        ...
        this.descendants = {
            ...
            folders: descendants.filter(d => d.type === DriveItemType.FOLDER).length,
     ...
     */
    private async getAllDescendants(queryRunner?: QueryRunner): Promise<DriveItem[]> {
        ...
        while (queue.length > 0) {
            ...
            const children = await repository.find({ where: { parentId: currentId } });
