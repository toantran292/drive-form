...
export class DriveService {
  ...
  async getUploadUrl(userId: string, body: MulterFile) {
    if (!ALLOWED_MIME_TYPES.includes(body.mimetype)) {
      throw new BadRequestException(
        'File type not allowed. Only images and PDF files are supported.',
      );
    }
  ...
  async saveFile(
    ...
    if (parentId) {
      ...
      if (parentFolder) {
        await parentFolder.updateDescendantsInfo(
          this.driveItemRepository.manager.connection.createQueryRunner(),
        );
    ...
    try {
      ...
      if (shareData.emails?.length) {
        ...
        if (sharedUsers.length === 0) {
          throw new BadRequestException(
            'No valid users found for the provided emails',
          );
        }
        ...
        for (const user of sharedUsers) {
          const existingShare = driveItem.sharedWith.find(
      ...
      if (driveItem.type === DriveItemType.FOLDER) {
        ...
        for (const userId of driveItem.sharedWith.map((s) => s.userId)) {
          await driveItem.propagateSharing(userId, 'read', queryRunner);
        }
      }
  ...
  async listUserFiles(userId: string, parentId?: string) {
    try {
      const query = this.driveItemRepository
        .createQueryBuilder('item')
        .leftJoinAndSelect('item.form', 'form')
        .where(
          new Brackets((qb) => {
            qb.where('item.ownerId = :userId', { userId }).orWhere(
  ...
  private hasReadAccess(driveItem: DriveItem, userId: string): boolean {
    ...
    return (
      Array.isArray(driveItem.sharedWith) &&
      driveItem.sharedWith.some((share) => share.userId === userId)
    );
  }
  ...
  async createForm(userId: string, data: CreateFormDto, parentId?: string) {
    ...
    try {
      ...
      const savedDriveItem = await queryRunner.manager.save(
        DriveItem,
        driveItem,
      );
  ...
  async updateForm(
    ...
    updateData.isPublic =
      updateData.settings.isPublished ||
      updateData.settings.allowAnonymous ||
      false;
  ...
  async submitFormResponse(formId: string, data: FormResponseData) {
    try {
      ...
      form.analytics = {
        totalResponses: (form.analytics?.totalResponses || 0) + 1,
        responsesByDate: {
          ...(form.analytics?.responsesByDate || {}),
          [data.submittedAt.toISOString().split('T')[0]]:
            (form.analytics?.responsesByDate?.[
              data.submittedAt.toISOString().split('T')[0]
            ] || 0) + 1,
        },
        questions: form.analytics?.questions || {},
        averageCompletionTime: form.analytics?.averageCompletionTime || 0,
        completionRate: form.analytics?.completionRate || 0,
      };

      // Cập nhật analytics cho từng câu hỏi
      if (form.questions) {
        for (const question of form.questions) {
          // Khởi tạo analytics cho câu hỏi nếu chưa có
          ...
          if (answer) {
            ...
            if (
              ...
              // Xử lý câu trả lời dạng multiple/single choice
              const optionIds = Array.isArray(answer.value)
                ? answer.value
                : [answer.value];

              // Chuyển validOptionIds thành Set để tối ưu việc kiểm tra
              const validOptionIdsSet = new Set(
                question.options?.map((opt) =>
                  typeof opt === 'string' ? opt : (opt as { id: string }).id,
                ) || [],
              );

              // Chỉ đếm các optionId hợp lệ, sử dụng Set.has() thay vì Array.includes()
              optionIds
                .filter((optionId) => validOptionIdsSet.has(optionId))
                .forEach((optionId) => {
                  questionAnalytics.options[optionId] =
                    (questionAnalytics.options[optionId] || 0) + 1;
                });
      ...
      form.analytics.completionRate =
        (form.analytics.completionRate * (form.analytics.totalResponses - 1) +
          completionRate) /
      ...
      // Xóa relation responses để tránh circular reference
