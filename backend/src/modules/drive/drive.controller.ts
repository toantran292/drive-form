...
export class DriveController {
    ...
    async getUploadUrl(
        ...
        try {
            const { url, path } = await this.driveService.getUploadUrl(req.user.uid, body);
            return {
                success: true,
                url,
                path
    ...
    async saveFile(
        ...
        try {
            const result = await this.driveService.saveFile(fileInfo, fileInfo.storagePath, req.user.uid, parentId);
            return {
                success: true,
                file: {
                    id: result.file.id,
    ...
    async listFiles(
        ...
        try {
            const items = await this.driveService.listUserFiles(req.user.uid, parentId);
    ...
    async previewFile(
        ...
        try {
            ...
            const { url, mimeType } = await this.driveService.getPreviewUrl(fileId, userId);
            return {
                success: true,
                url,
                mimeType,
    ...
    async downloadFile(
        ...
        try {
            const userId = req.user?.uid; // Optional: người dùng có thể đã đăng nhập hoặc chưa
            const { url, filename } = await this.driveService.getDownloadUrl(fileId, userId);
            return {
                success: true,
                url,
                filename
    ...
    async getSharedFile(@Param('shareId') shareId: string, @Request() req) {
        try {
            const userId = req.user?.uid; // Optional: người dùng có thể đã đăng nhập hoặc chưa
            const file = await this.driveService.getSharedFileByShareId(shareId, userId);

            // Lấy URL xem trước và tải xuống
            const { url: previewUrl } = await this.driveService.getPreviewUrl(file.id, userId);
            const { url: downloadUrl } = await this.driveService.getDownloadUrl(file.id, userId);
    ...
    async createForm(
        ...
        try {
            const { item } = await this.driveService.createForm(req.user.uid, createFormDto, parentId);
            return {
                success: true,
                item: {
                    id: item.id,
        ...
        try {
            ...
            if (form.settings?.limitOneResponsePerUser) {
                if (userId) {
                    ...
                    const hasResponded = form.responses?.some(response => response.respondentId === userId);
                    if (hasResponded) {
                        canSubmit = false;
                        message = 'You have already submitted a response to this form';
                    }
                } else {
                    // Nếu form yêu cầu limit và user chưa đăng nhập
                    canSubmit = false;
    ...
    async submitFormResponse(
        ...
        try {
            ...
            if (form.settings?.collectEmail && !response.respondentEmail) {
            ...
            // Validate answers format theo type của question
            for (const answer of response.answers) {
                ...
                switch (question.type) {
                    case QuestionType.MULTIPLE_CHOICE:
                        // Kiểm tra value có nằm trong options không
                        if (!Array.isArray(answer.value)) {
                            throw new BadRequestException(`Invalid answer format for multiple choice question "${question.title}"`);
                        }
                        const validOptions = question.options?.map(opt => (opt as any).value) || [];
                        const invalidOptions = answer.value.filter(v => !validOptions.includes(v));
                        if (invalidOptions.length > 0) {
                            throw new BadRequestException(`Invalid options selected for question "${question.title}"`);
                        }
                        break;

                    case QuestionType.SINGLE_CHOICE:
                        // Kiểm tra value có nằm trong options không
                        if (Array.isArray(answer.value) || !question.options?.some(opt => (opt as any).value === answer.value)) {
                            throw new BadRequestException(`Invalid option selected for question "${question.title}"`);
                        }
                        break;
                    ...
                    case QuestionType.PARAGRAPH:
                        if (Array.isArray(answer.value)) {
                            throw new BadRequestException(`Invalid answer format for text question "${question.title}"`);
                        }
                        // Có thể thêm validate độ dài text ở đây nếu cần
                        break;
                }
            }
    ...
    async createFolder(
        ...
        try {
            const folder = await this.driveService.createFolder(createFolderDto, req.user);
            return {
                success: true,
                folder: {
                    id: folder.id,
    ...
    async getFolderPath(
        ...
        try {
            const path = await this.driveService.getFolderPath(folderId, req.user.uid);
            return {
                success: true,
                path
            };
    ...
    async moveItem(
        ...
        try {
            ...
            throw new InternalServerErrorException(`Failed to move item: ${error.message}`);
        }
    }
