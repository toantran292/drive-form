...
export class FormService {
    ...
    async getForm(id: string, userId?: string) {
        ...
        if (userId && form.ownerId !== userId && !form.sharedWith.some(share => share.userId === userId)) {
    ...
    async updateForm(id: string, userId: string, updateFormDto: UpdateFormDto) {
        ...
        if (form.ownerId !== userId && !form.sharedWith.some(share => share.userId === userId && share.permission === 'edit')) {
    ...
    async shareForm(id: string, ownerId: string, userId: string, permission: 'view' | 'edit') {
    ...
    async submitForm(id: string, answers: { questionId: string; value: string | string[] }[]) {
        ...
        const missingRequired = requiredQuestions.filter(q =>
            !answers.some(a => a.questionId === q.id && a.value !== '' && a.value.length > 0)
        );
    ...
    async getFormResponses(id: string, userId: string) {
        ...
        if (form.ownerId !== userId && !form.sharedWith.some(share => share.userId === userId)) {
            throw new ForbiddenException('You do not have permission to view responses');
        }
